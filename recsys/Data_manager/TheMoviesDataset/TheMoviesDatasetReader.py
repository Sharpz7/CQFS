...
from recsys.Data_manager.Movielens.Movielens20MReader import \
...
class TheMoviesDatasetReader(DataReader):
    ...
    def _load_from_original_file(self):
        ...
        ) = self._loadICM_credits(credits_path, header=True, if_new_item="add")
        ...
        (
            ...
            self.item_original_ID_to_index,
        ) = self._loadICM_metadata(
            metadata_path, header=True, if_new_item="add"
        )
        ...
        ICM_credits = reshapeSparse(
            ICM_credits, (n_items, ICM_credits.shape[1])
        )

        # IMPORTANT: ICM uses TMDB indices, URM uses movielens indices
        # Load index mapper
        movielens_id_to_tmdb, tmdb_to_movielens_id = (
            self._load_item_id_mappping(
        ...
        self.loaded_ICM_mapper_dict = {
            "ICM_credits": tokenToFeatureMapper_ICM_credits,
    ...
    def _load_item_id_mappping(self, movielens_tmdb_id_map_path, header=True):
        ...
        movielens_tmdb_id_map_file = open(
            movielens_tmdb_id_map_path, "r", encoding="utf8"
        )
    ...
    def _replace_tmdb_id_with_movielens(self, tmdb_to_movielens_id):
        ...
        self.item_index_to_original_ID = invert_dictionary(
            self.item_original_ID_to_index
        )
    ...
    def _loadICM_credits(self, credits_path, header=True, if_new_item="add"):
        ...
        for newCredits in parser_credits:
            ...
            try:
                ...
                print(
                    "TheMoviesDatasetReader: Exception while parsing: '{}', skipping".format(
                        str(e)
                    )
                )
    ...
    def _new_loadICM_credits(
        self, credits_path, header=True, if_new_item="add"
    ):
        ...
        original_cast_crew_df = pd.concat(
            [original_cast_df, original_crew_df], sort=False, ignore_index=True
        )
        ...
        more_than_5_items_df = name_job_cast_crew_df[
            name_job_count_df["movie_id"] > 5
        ].reset_index()

        directors = more_than_5_items_df[
            more_than_5_items_df["job"] == "Director"
        ]
        s_time = time.time()
        for d in range(directors.shape[0]):
            ...
            ICM_builder.add_data_lists(
                row_list, col_list, data_list_to_add=[1.0]
            )
    ...
    def get_filtered_ICM_credits(
        ...
        ICM_builder = IncrementalSparseMatrix_FilterIDs(
            preinitialized_col_mapper=None,
            on_new_col="add",
            preinitialized_row_mapper=item_mapper,
            on_new_row=if_new_item,
        )
        ...
        if k > 0:
            name_job_cast_crew_df = name_job_cast_crew_df[
                name_job_count_df["movie_id"] >= k
            ].reset_index()

        if categories is not None and len(categories) > 0:
            name_job_cast_crew_df = name_job_cast_crew_df[
                name_job_cast_crew_df["job"].isin(categories)
            ]

        # IMPORTANT: ICM uses TMDB indices, URM uses movielens indices
        # Load index mapper
        movielens_tmdb_id_map_path = dataFile.extract(
            "links.csv", path=decompressed_zip_file_folder + "decompressed/"
        )
        movielens_id_to_tmdb, tmdb_to_movielens_id = (
            self._load_item_id_mappping(
                movielens_tmdb_id_map_path, header=True
    ...
    def _loadICM_metadata(self, metadata_path, header=True, if_new_item="add"):
        ...
        for newMetadata in parser_metadata:
            ...
            if len(newMetadata) < 22:
                # Sono 6, ragionevole
                print(
                    "TheMoviesDatasetReader: Line too short, possible unwanted new line character, skipping"
                )
                continue
            ...
            if newMetadata[12]:
                ...
                for prod_company in prod_companies:
                    token_list.append(
                        "production_company_" + str(prod_company["id"])
                    )
            ...
            if newMetadata[13]:
                ...
                for prod_country in prod_countries:
                    token_list.append(
                        "production_country_" + prod_country["iso_3166_1"]
                    )
            ...
            if newMetadata[17]:
                ...
                for spoken_lang in spoken_langs:
                    token_list.append(
                        "spoken_lang_" + spoken_lang["iso_639_1"]
                    )
