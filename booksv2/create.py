...
def add_custom_column(df, column_type, column_name):
    if column_type == ColumnType.SPARSE:
        ...
        df[column_name] = np.random.choice([0, 1, np.nan], size=len(df), p=[0.8, 0.1, 0.1])
        ...
        df[column_name] = df['Year-Of-Publication'] + np.random.normal(0, 1, size=len(df))
...
with tempfile.TemporaryDirectory() as temp_dir:
    ...
    shutil.copy('Ratings.csv', temp_dir)
