...
def baseline_CBF(
    ...
    ICM_train, original_ICM_train = data_loader.get_ICM_train_from_name(
        ICM_name, return_original=True
    )
    ...
    train_warm_item_mask = np.arange(len(train_warm_item_mask))[
        train_warm_item_mask
    ]

    # Obtain the array of train and validation item indices, to be ignored during testing
    train_validation_warm_item_mask = (
        np.ediff1d(URM_train_last_test.tocsc().indptr) != 0
    )
    train_validation_warm_item_mask = np.arange(
        len(train_validation_warm_item_mask)
    )[train_validation_warm_item_mask]
    ...
    evaluator_validation = EvaluatorHoldout(
        URM_validation, cutoff_list=[10], ignore_items=train_warm_item_mask
    )
    evaluator_test = EvaluatorHoldout(
        URM_test,
        cutoff_list=[5, 10, 20, 50],
    ...
    runParameterSearch_Content(
        ...
        ICM_last_test=original_ICM_train,
        n_cases=n_cases,
