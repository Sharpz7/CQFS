...
from utils.statistics import BQM_statistics, similarity_statistics
...
class CQFS:
    ...
    def __init__(
        ...
        if re.match(".*/.*ICM.*/.*Recommender.*/", base_folder_path) is None:
            self.__print(
                "[WARNING] base_folder_path has a custom format, we suggest to use the following one for "
        ...
        self.base_folder_path = (
            base_folder_path
            if base_folder_path[-1] == "/"
            else f"{base_folder_path}/"
        )
        ...
        if not self.saved_models["K"] or not self.saved_models["E"]:
            ...
            assert (
                S_CF.shape == S_CBF.shape
            ), "The two sparse matrices have different shapes."
            assert S_CF.shape == (
                self.n_items,
                self.n_items,
            ), "The similarity matrices do not have the right shape."
            ...
            assert (
                self.K.nnz + self.E.nnz == S_CBF.nnz
            ), "The number of items to keep and to penalize is not correct."

            S_union = S_CF_bool + S_CBF_bool

            self.statistics = similarity_statistics(
                S_CF,
                S_CBF,
                S_intersection,
                S_union,
        ...
        assert (
            qpu_topology in self.ACCEPTED_TOPOLOGIES
        ), f"QPU topology should be one of {self.ACCEPTED_TOPOLOGIES}."
        ...
        if solver_class is DWaveSampler:
            ...
            self.sampler = DWaveSampler(
                solver={"topology__type": qpu_topology}
            )
            self.embedder = CQFSEmbedding(
                target_sampler=self.sampler,
            ...
            assert (
                solver_class in self.ACCEPTED_SOLVERS
            ), f"Solver class should be one of {self.ACCEPTED_SOLVERS}."
            self.solver = solver_class()
    ...
    def __p_to_k(p, n_features):
        assert p is not None, (
            "Please, choose a selection percentage."
        ...
        if 1 < p <= 100:
            ...
            raise ValueError(
                "Percentage value should be between 0 and 1 or between 0 and 100."
            )
    ...
    def fit(self, alpha=1, beta=1, vartype="BINARY", save_FPM=False):
        ...
        if self.BQMs.get(fitID) is None:
            ...
            self.timings["avg_IPM_time"] = (
                total_IPM_time + IPM_time
            ) / n_experiments
            self.timings["avg_FPM_time"] = (
                total_FPM_time + FPM_time
            ) / n_experiments
            self.timings["avg_BQM_time"] = (
                total_BQM_time + BQM_time
            ) / n_experiments
            self.timings["avg_QUBO_time"] = (
            ...
            try:
                ...
                self.__print(
                    f"[{fitID}] Found a previously saved {FPM_statistics_file} file."
                ...
                FPM_statistics = BQM_statistics(
                    linear, quadratic_data, FPM.shape, prefix="FPM_"
                )
                self.__print(
                    f"[{fitID}] Computed {FPM_statistics_file}. Saving to file."
                )
                experiment_dataIO.save_data(
                    FPM_statistics_file, FPM_statistics
                )
            ...
            if save_FPM:
                ...
                try:
                    ...
                    self.__print(
                        f"[{fitID}] Found a previously saved {FPM_file} file."
    ...
    def __get_selection_dataIO(self, expID):
        selection_folder_path = (
            f"{self.base_folder_path}{expID}/{self.selection_type}/"
        )
    ...
    def __load_selection(self, expID):
        ...
        try:
            selection = selection_dataIO.load_data(self.selection_type)[
                "selection"
            ]
    ...
    def __compute_and_save_BQM_statistics(
        self, BQM, prefix, experiment_dataIO
    ):
        ...
        try:
            ...
            self.__print(
                f"Found a previously saved {file_name} file. Skipping statistics computation."
            )
        except FileNotFoundError:
            BQM_linear, (_, _, BQM_quadratic_data), _ = BQM.to_numpy_vectors()
            shape = (BQM.num_variables, BQM.num_variables)
            statistics = BQM_statistics(
                BQM_linear, BQM_quadratic_data, shape, prefix=prefix
            )
    ...
    def select_p(
        ...
        if BQM is None:
            ...
            if BQM is None:
                self.fit(
                    alpha=alpha, beta=beta, vartype=vartype, save_FPM=save_FPM
                )
                BQM = self.BQMs.get(fitID)

        BQM_time = time.time()
        BQM_k = dimod.generators.combinations(
            self.n_features, k, strength=combination_strength, vartype=vartype
        )
        ...
        self.__compute_and_save_BQM_statistics(
            BQM_k, "BQM_", experiment_dataIO
        )
        ...
        if normalize:
            ...
            self.__compute_and_save_BQM_statistics(
                BQM_k, "BQM_norm_", experiment_dataIO
            )
        ...
        if self.qpu_only:
            ...
            embedding = self.embedder.get_embedding(
                BQM_k, expID, resume_from_saved=True
            )
            self.solver = CQFSSampler(
                embedding,
                target_sampler=self.sampler,
        ...
        n_experiments = self.timings["n_select_experiments"][
            self.selection_type
        ]
        total_response_time = (
            self.timings["avg_response_time"][self.selection_type]
            * n_experiments
        )
        n_experiments += 1
        self.timings["n_select_experiments"][
        ...
        self.__print(
            f"[{expID}] Selected {selection.sum()} features in {response_time} sec."
        )
