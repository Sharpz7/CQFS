...
from recsys.Data_manager import TheBooksV2DatasetReader
...
def main():
    ...
    data_reader = TheBooksV2DatasetReader()
    ...
    (
        ...
        URM_train_warm,
        URM_validation_warm,
    ) = data_splitter.get_both_splits()
    data_splitter.get_statistics_URM_warm()

    # Get the warm items URM
    URM_train_validation = merge_sparse_matrices(
        URM_train, URM_validation
    ).tocsr()
    URM_train_validation_warm = merge_sparse_matrices(
        URM_train_warm, URM_validation_warm
    ).tocsr()
    assert (
        URM_train_validation.nnz == URM_train_validation_warm.nnz
    ), "WRONG SPLIT!"
    assert np.equal(
        URM_train_validation.indptr, URM_train_validation_warm.indptr
    ).all(), "WRONG SPLIT!"
    assert np.equal(
        URM_train_validation.indices, URM_train_validation_warm.indices
    ).all(), "WRONG SPLIT!"
